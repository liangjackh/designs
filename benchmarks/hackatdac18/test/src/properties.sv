module jg_properties (
    ...
    HACKDAC_p3: assert property (
        (~((top_wrapper.riscv_core.cs_registers_i.priv_lvl_n == top_wrapper.riscv_core.cs_registers_i.PRIV_LVL_M) && 
           (top_wrapper.riscv_core.cs_registers_i.mstatus_n.mpp == top_wrapper.riscv_core.cs_registers_i.PRIV_LVL_U)))
    );
    ...
    HACKDAC_p11: assert property (
        (~((top_wrapper.riscv_core.debug_unit_i.dbg_halt != 1) && 
           (top_wrapper.riscv_core.debug_unit_i.rdata_sel_n == top_wrapper.riscv_core.RD_DBGS)))
    );
    ...
    HACKDAC_p14: assert property (
        (((top_wrapper.riscv_core.ex_stage_i.alu_i.vector_mode_i == top_wrapper.riscv_core.ex_stage_i.alu_i.VEC_MODE16) || 
          (top_wrapper.riscv_core.ex_stage_i.alu_i.vector_mode_i == top_wrapper.riscv_core.ex_stage_i.alu_i.VEC_MODE8)) |-> 
    ...
    // 注意：这里的 rst 如果在顶层没有定义，请确保它映射到正确的信号，比如 !rstn_top。这里暂时按你的原始名称使用。
